...
//export connectStart
func connectStart() C.Result {
	code, err := api.ConnectStart()
	return cResult(code, err)
}

//export connectListen
func connectListen(code C.int) C.Result {
	ok, identity, secret, err := api.ConnectListen(int(code))
	return cResult(map[string]any{
		"ok":       ok,
		"identity": identity,
		"secret":   secret,
	}, err)
...
type connectShareArg struct {
	Code         int
	PeerIdentity security.Identity
	PoolNames    []string
}

//export connectShare
func connectShare(data *C.char) C.Result {
	var arg connectShareArg
	err := json.Unmarshal([]byte(C.GoString(data)), &arg)
	if core.IsErr(err, "cannot unmarshal connectShare argument: %v") {
		return cResult(nil, err)
	}

	err = api.ConnectShare(arg.Code, arg.PeerIdentity, arg.PoolNames)
	return cResult(nil, err)
}

//export connectRequest
func connectRequest(code C.int) C.Result {
	secret, err := api.ConnectRequest(int(code))
	return cResult(secret, err)
}

//export connectComplete
func connectComplete(code C.int) C.Result {
	ready, tokens, err := api.ConnectComplete(int(code))
	return cResult(map[string]any{
		"ready":  ready,
		"tokens": tokens,
	}, err)
