...
func OpenWebDAV(connectionUrl string) (Store, error) {
	...
	if core.IsErr(err, nil, "invalid url '%s': %v", connectionUrl) {
		return nil, err
	}
	...
	default:
		core.IsErr(os.ErrInvalid, nil, "invalid scheme %s: %v", u.Scheme)
	...
	err = c.Connect()
	if core.IsErr(err, nil, "cannot connect to WebDAV '%s': %v", connectionUrl) {
		return nil, err
	}
...
func (w *WebDAV) Read(name string, rang *Range, dest io.Writer, progress chan int64) error {
	...
	if core.IsErr(err, nil, "cannot read WebDAV file %s: %v", p) {
		return err
	}
	...
	if rang == nil {
		...
		if core.IsErr(err, nil, "cannot discard %n bytes in range GET on %s: %v", rang.From, p) {
			return err
		}
		if written != rang.From {
			core.IsErr(io.ErrShortWrite, nil, "Cannot read %d bytes in GET %s: %v")
	...
	if err != nil && err != io.EOF {
		core.IsErr(err, nil, "cannot read from GET response on %s: %v", p)
		return err
	}
	r.Close()
	return nil
...
func (w *WebDAV) Write(name string, source io.ReadSeeker, progress chan int64) error {
	...
	if core.IsErr(err, nil, "cannot write WebDAV file %s: %v", p) {
		return err
	}
...
func (w *WebDAV) ReadDir(dir string, f Filter) ([]fs.FileInfo, error) {
	...
	if core.IsErr(err, nil, "cannot read WebDAV folder %s: %v", p) {
		return nil, err
	}
