...
func OpenLocal(connectionUrl string) (Store, error) {
	...
	if core.IsErr(err, nil, "invalid URL: %v") {
		return nil, err
	}
...
func (l *Local) Read(name string, rang *Range, dest io.Writer, progress chan int64) error {
	...
	if core.IsErr(err, nil, "cannot read from %s/%s:%v", l, name) {
		return err
	}
...
func (l *Local) Write(name string, source io.ReadSeeker, progress chan int64) error {
	...
	err := createDir(n)
	if core.IsErr(err, nil, "cannot create parent of %s: %v", n) {
		return err
	}

	f, err := os.Create(n)
	if core.IsErr(err, nil, "cannot create file on %v:%v", l) {
		return err
	}
	defer f.Close()
