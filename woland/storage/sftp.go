...
func (s *SFTP) Read(name string, rang *Range, dest io.Writer, progress chan int64) error {
	...
	if os.IsNotExist(err) || core.IsErr(err, nil, "cannot open file on sftp server %v:%v", s) {
		return err
	}
	...
	if err != io.EOF && core.IsErr(err, nil, "cannot read from %s/%s:%v", s, name) {
		return err
	}
...
func (s *SFTP) Write(name string, source io.ReadSeeker, progress chan int64) error {
	...
	if core.IsErr(err, nil, "cannot create SFTP file '%s': %v", name) {
		return err
	}

	_, err = io.Copy(f, source)
	core.IsErr(err, nil, "cannot write SFTP file '%s': %v", name)
	return err
}
...
func (s *SFTP) Delete(name string) error {
	...
	stat, err := s.c.Stat(n)
	if core.IsErr(err, nil, "cannot stat %s in Delete: %v", n) {
		return err
	}
	...
	err = s.c.Remove(n)
	core.IsErr(err, nil, "cannot delete %s in Delete: %v", n)
	return nil
}
