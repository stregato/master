...
func (s *Safe) List(zoneName string, listOptions ListOptions) ([]Header, error) {
...
func sync(safeName, zoneName string, store storage.Store, keys map[uint64][]byte) error {
	var touch time.Time
	var err error
	...
	var lastYMD string
	err = sql.QueryRow("GET_LAST_YMD", sql.Args{"safe": safeName, "zone": zoneName}, &lastYMD)
	if err != sql.ErrNoRows && core.IsErr(err, "cannot get last YMD: %v", err) {
		return err
	}
	ids, err := getBodyIds(safeName, zoneName, lastYMD)
...
func saveHeaderToDB(safeName, zoneName, ymd string, header Header) error {
...
func getBodyIds(safeName, zoneName, ymd string) (map[uint64]bool, error) {
	ids := map[uint64]bool{}
	rows, err := sql.Query("GET_FILE_BODY_ID", sql.Args{"safe": safeName, "zone": zoneName, "ymd": ymd})
	if core.IsErr(err, "cannot get body ids: %v", err) {
		return nil, err
	}
