...
func getUsers(creatorId string, trail []PermissionChange) (users Users, clearTrail []PermissionChange) {
	sort.Slice(trail, func(i, j int) bool {
		return trail[i].ModTime.Before(trail[j].ModTime)
	})

	users = map[string]Permission2{creatorId: PermissionAdmin2}
	for _, change := range trail {
		valid := isValidPermissionChange(change)
		if valid && users[change.By]&PermissionAdmin2 != 0 {
			for userId, permission := range change.Users {
