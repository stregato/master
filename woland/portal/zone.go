...
func (s *Portal) Zones() ([]string, error) {
	var zones []string
	for z := range s.zones {
...
func (s *Portal) AddZone(zoneName string, creatorId string) error {
...
func (s *Portal) CreateZone(zoneName string, users Users) error {
	_, err := s.store.Stat(path.Join(zonesDir, zoneName))
	if err == nil {
		return ErrZoneExist
	}
...
func readZone(currentUser security.Identity, store storage.Store, portalName string, zoneName string,
	zone *Zone) error {
