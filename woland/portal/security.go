...
func (s *Portal) SetUsers(zoneName string, users Users) error {
...
func writeZoneAddition(currentUser security.Identity, store storage.Store, portalName string, zoneName string, zone *Zone) error {
	...
	err = setZoneToDB(portalName, zoneName, *zone)
...
func (s *Portal) GetUsers(zoneName string) (Users, error) {
	zone, ok := s.zones[zoneName]
	if !ok {
